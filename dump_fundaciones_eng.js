...
const engDir = path.join(__dirname, 'app', 'src', 'data', 'clubes', 'inglaterra');
const files = fs.readdirSync(engDir).filter(f => f.endsWith('.json'));
let output = '';

for (const file of files) {
    const data = JSON.parse(fs.readFileSync(path.join(engDir, file), 'utf8'));
...
fs.writeFileSync('temp_fundaciones_eng.txt', output, 'utf8');
