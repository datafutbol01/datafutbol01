...
async function scrapeDfbPokal() {
    try {
        console.log("Iniciando scraper para DFB Pokal (Alemania)...");
        ...
        if (fixtures) {
            fixtures.forEach(match => {
                ...
                // En DFB Pokal puede decir "Round of 16", "8th Finals", "Quarter-finals", "Semi-finals", "Final"
                ...
                else if (round.includes("final") && !round.includes("quarter") && !round.includes("semi") && !round.includes("8th")) stage = "final";
        ...
        const outDir = path.join(__dirname, '../app/src/data/copas/alemania');
        if (!fs.existsSync(outDir)) {
            fs.mkdirSync(outDir, { recursive: true });
        }

        fs.writeFileSync(path.join(outDir, '2026.json'), JSON.stringify(copaData, null, 2));
        console.log("Archivo 2026.json (DFB Pokal) generado con éxito en app/src/data/copas/alemania!");
