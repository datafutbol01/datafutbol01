...
async function processFolders() {
    ...
    for(const f of folders) {
        if(f.toLowerCase() <= 'bradford city') continue; // Already processed
        if(f.toLowerCase() > 'clapham rovers') continue; // Only process up to Clapham Rovers
        ...
        // SPECIAL CASE: CLAPHAM ROVERS
        // User requested to verify the shields and avoid the misspelled "Claphan".
        // Instead of trusting the contents, let's delete them and fetch the true official Wikipedia logo directly.
        if (slug === 'clapham-rovers') {
             files.forEach(file => fs.unlinkSync(path.join(sourcePath, file)));
             console.log('Removed user-provided Clapham files to ensure authenticity.');
             const destWiki = path.join(publicEscudos, 'clapham-rovers.png');
             
             await new Promise((resolve) => {
                 https.get('https://upload.wikimedia.org/wikipedia/commons/2/23/Clapham_rovers_fc.png', (res) => {
                     const fileStream = fs.createWriteStream(destWiki);
                     res.pipe(fileStream);
                     fileStream.on('finish', () => {
                         fileStream.close();
                         console.log('Downloaded verified Clapham Rovers shield from Wikipedia!');
