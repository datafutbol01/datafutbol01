...
const engClubesDir = path.join(srcDir, 'clubes', 'inglaterra');
...
// 1. Base Clubs
if (fs.existsSync(engClubesDir)) {
    fs.readdirSync(engClubesDir).filter(f => f.endsWith('.json')).forEach(f => {
        try {
            const data = JSON.parse(fs.readFileSync(path.join(engClubesDir, f), 'utf8'));
            if (data.id) {
                baseClubs.add(data.id);
                // Si tiene URL estatica como Barracas, la marcamos
...
try {
    const tempPath = path.join(ligasDir, 'inglaterra_temporadas.json');
...
try {
    const copasPath = path.join(ligasDir, 'inglaterra_copas.json');
...
// 4. H2H Rivals
try {
    const h2hPath = path.join(ligasDir, 'inglaterra_enfrentamientos.json');
...
let output = `=== REPORTE REAL PUNTO 2 (INGLATERRA) ===\n`;
...
output += `Faltan escudos de estos foráneos o históricos: ${missingOthers.length}\n`;
if (missingOthers.length > 0) output += `> FALTAN EXTINTOS: ${missingOthers.join(', ')}\n`;

fs.writeFileSync('reporte_punto2_eng.txt', output, 'utf8');
