...
function processFolders() {
    ...
    for(const f of folders) {
        if(f.toLowerCase() < 'oxford university') continue; // Only from Oxford University
        if(f.toLowerCase() > 'real betis') continue; // Stop after Real Betis
        ...
        if(isBaseClub) {
            ...
            for(const file of files) {
                ...
                // Copy without throwing if it exists, it safely overwrites.
                try { fs.copyFileSync(oldLocation, newLocation); } catch(ex){}
                
                const fileUrl = `/escudos_historicos/${slug}/${cleanDestName}`;
                // Double check to absolutely avoid duplicates
            ...
            // Clean up exact year duplicates (e.g., if one was downloaded and there already was an exact one for that year, though we keep _full variations)
            const uniqueYears = new Set();
            data.evolucion_escudos = data.evolucion_escudos.filter(e => {
                 // For now, allow multiple per year (like a minor change within the year), but standard URL checking solves duplicate files
                 return true; 
            });
