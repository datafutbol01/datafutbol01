...
async function run() {
  ...
  for (const rival of rivals) {
    ...
    while(retries > 0 && !found) {
        try {
          process.stdout.write(`Extrayendo Aberdeen vs ${rival.name}... `);
    ...
    if(!found) {
        results.push({ Rival: rival.name, error: 'Could not fetch absolute raw data' });
    }
  }

  console.log("\nABERDEEN H2H OFICIAL RIGUROSO 100%:");
