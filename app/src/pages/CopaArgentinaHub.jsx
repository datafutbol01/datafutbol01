...
export default function CopaArgentinaHub() {
    const seasons = [
        { year: 2026, host: 'Argentina' }
    ];
    ...
    useEffect(() => {
        ...
        let importPromise;
        switch (selectedYear) {
            case 2026: importPromise = import('../data/copas/argentina/2026.json'); break;
        ...
        <div style={{ minHeight: '100vh', background: 'var(--bg-main)', position: 'relative' }}>
                    ...
                    <div style={{
                        ...
                        <motion.img
                            ...
                            style={{
                                ...
                                filter: 'contrast(1.2) saturate(1.3) brightness(0.6) sepia(20%) hue-rotate(180deg)', // Un toque celeste para argentina
            ...
            <div style={{ textAlign: 'center', marginBottom: '2rem', padding: '0 2rem' }}>
                ...
                <p style={{ color: 'var(--text-muted)', fontSize: '1.2rem', maxWidth: '800px', margin: '0 auto', fontWeight: '300' }}>
                    Revive la Copa Argentina. Repasa las llaves eliminatorias y las estadísticas históricas del torneo federal más apasionante.
                </p>
            </div>
                    ...
                    {selectedYear === null ? (
                        ...
                        >
                            ...
                            <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: '1.2rem', maxWidth: '600px', margin: '0 auto 3rem', lineHeight: '1.6' }}>
                                Selecciona un año en el carrusel superior para revivir cada partido de la Copa Argentina.
                            </p>
                        </motion.div>
                    ) : (
                        <motion.div
                                                ...
                                                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                                                    ...
                                                    {!cupData.knockoutPhase ? (
                                                        ...
                                                        <div className="glass-panel hide-scrollbar" style={{ padding: '2rem', borderRadius: '16px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', overflowX: 'auto', scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                                                            <div style={{ display: 'flex', justifyContent: 'flex-start', gap: '1.5rem', minWidth: '1200px' }}>
                                                                
                                                                {/* COL 1: 32vos */}
                                                                {cupData.knockoutPhase.treintaYDos && cupData.knockoutPhase?.treintaYDos?.length > 0 && (
                                                                    <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-around', gap: '1rem', width: '220px', position: 'relative' }}>
                                                                        <div style={{ textAlign: 'center', fontSize: '0.7rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '1px' }}>32vos de Final</div>
                                                                        {cupData.knockoutPhase.treintaYDos.map((m, i) => <div key={i}>{renderMatch(m)}</div>)}
                                                                    </div>
                                                                )}

                                                                {/* COL 2: 16vos */}
                                                                {cupData.knockoutPhase.dieciseis && cupData.knockoutPhase?.dieciseis?.length > 0 && (
                                                                    <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-around', gap: '1rem', width: '220px', position: 'relative' }}>
                                                                        <div style={{ textAlign: 'center', fontSize: '0.7rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '1px' }}>16vos de Final</div>
                                                                        {cupData.knockoutPhase.dieciseis.map((m, i) => <div key={i}>{renderMatch(m)}</div>)}
                                                                    </div>
                                                                )}
                                                                ...
                                                                {cupData.knockoutPhase.octavos && cupData.knockoutPhase?.octavos?.length > 0 && (
                                                                    <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-around', gap: '1rem', width: '220px', position: 'relative' }}>
                                                                        <div style={{ textAlign: 'center', fontSize: '0.7rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '1px' }}>Octavos</div>
                                                                ...
                                                                {cupData.knockoutPhase.cuartos && cupData.knockoutPhase?.cuartos?.length > 0 && (
                                                                    <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-around', width: '220px', position: 'relative' }}>
                                                                        <div style={{ textAlign: 'center', fontSize: '0.7rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '1px' }}>Cuartos</div>
                                                                ...
                                                                {cupData.knockoutPhase.semifinal && cupData.knockoutPhase?.semifinal?.length > 0 && (
                                                                    <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-around', width: '220px', position: 'relative' }}>
                                                                        <div style={{ textAlign: 'center', fontSize: '0.7rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '1px' }}>Semis</div>
