...
import { ChevronLeft, ChevronRight, Trophy, AlertCircle, ArrowLeft } from 'lucide-react';
import CryptoJS from 'crypto-js';

export default function LigaHolandaHub() {
    const seasons = [
        { year: 2026, host: 'Países Bajos' }
    ];
    ...
    useEffect(() => {
        ...
        let importPromise;
        switch (selectedYear) {
            case 2026: importPromise = import('../data/ligas/holanda/2026.json'); break;
            ...
            .then(module => {
                ...
                if (data && data.payload) {
                    try {
                        ...
                        console.error("Error decrypting liga data:", e);
                    }
                }
                setLigaData(data);
                setIsLoading(false);
    ...
    const bgImage = '/portada_mundiales_ai.png'; 
        ...
        <div style={{ minHeight: '100vh', background: 'var(--bg-main)', position: 'relative' }}>
                    ...
                    <div style={{
                        ...
                        <motion.img
                            ...
                            style={{
                                ...
                                filter: 'contrast(1.2) saturate(1.3) brightness(0.6) sepia(20%) hue-rotate(10deg)', // Un toque naranja para holanda
            ...
            <div style={{ textAlign: 'center', marginBottom: '2rem', padding: '0 2rem' }}>
                ...
                <p style={{ color: 'var(--text-muted)', fontSize: '1.2rem', maxWidth: '800px', margin: '0 auto', fontWeight: '300' }}>
                    Revive la Eredivisie de los Países Bajos. Repasa las tablas de posiciones y las estadísticas históricas.
                </p>
            </div>
                    ...
                    {selectedYear === null ? (
                        ...
                        >
                            ...
                            <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: '1.2rem', maxWidth: '600px', margin: '0 auto 3rem', lineHeight: '1.6' }}>
                                Selecciona un año en el carrusel superior para revivir las posiciones y estadísticas de la Eredivisie.
                            </p>
                        </motion.div>
                    ) : (
                        <motion.div
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(250px, 300px) 1fr', gap: '2rem' }}>
                                <div className="glass-panel" style={{ padding: '1rem', borderRadius: '16px', display: 'flex', flexDirection: 'column', flexWrap: 'nowrap', gap: '0.5rem', height: 'fit-content' }}>
                                    ...
                                    <button
                                        ...
                                        style={{ textAlign: 'left', padding: '1rem', background: activeTab === 'posiciones' ? 'rgba(251, 191, 36, 0.1)' : 'rgba(255,255,255,0.05)', color: activeTab === 'posiciones' ? 'var(--accent-gold)' : 'white', border: `1px solid ${activeTab === 'posiciones' ? 'var(--accent-gold)' : 'rgba(255,255,255,0.1)'}`, borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer', transition: 'all 0.2s' }}
                                    >
                                                ...
                                                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                                                    ...
                                                    {!ligaData.standings ? (
                                                        <div style={{ padding: '3rem', textAlign: 'center', color: 'var(--text-muted)', borderStyle: 'dashed', borderWidth: '1px', borderColor: 'rgba(255,255,255,0.1)', borderRadius: '16px' }}>
                                                            La tabla de posiciones se inyectará aquí próximamente.
                                                        </div>
                                                    ) : (
                                                        <div className="glass-panel hide-scrollbar" style={{ padding: '1.5rem', borderRadius: '16px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', overflowX: 'auto', scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                                                            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '700px' }}>
                                                                <thead>
                                                                    <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.1)', color: 'var(--text-muted)', fontSize: '0.85rem', textTransform: 'uppercase', letterSpacing: '1px' }}>
                                                                        <th style={{ padding: '1rem 0.5rem', textAlign: 'left', width: '40%' }}>Club</th>
                                                                        <th style={{ padding: '1rem 0.5rem', textAlign: 'center', color: 'var(--accent-gold)' }}>Pts</th>
                                                                        <th style={{ padding: '1rem 0.5rem', textAlign: 'center' }}>PJ</th>
                                                                        <th style={{ padding: '1rem 0.5rem', textAlign: 'center' }}>G</th>
                                                                        <th style={{ padding: '1rem 0.5rem', textAlign: 'center' }}>E</th>
                                                                        <th style={{ padding: '1rem 0.5rem', textAlign: 'center' }}>P</th>
                                                                        <th style={{ padding: '1rem 0.5rem', textAlign: 'center' }}>GF</th>
                                                                        <th style={{ padding: '1rem 0.5rem', textAlign: 'center' }}>GC</th>
                                                                        <th style={{ padding: '1rem 0.5rem', textAlign: 'center' }}>DIF</th>
                                                                    </tr>
                                                                </thead>
                                                                <tbody>
                                                                    {ligaData.standings.map((t, i) => (
                                                                        <tr key={t.team} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)', background: i === 0 ? 'rgba(251, 191, 36, 0.05)' : 'transparent', transition: 'background 0.2s' }} onMouseOver={e => e.currentTarget.style.background = 'rgba(255,255,255,0.05)'} onMouseOut={e => e.currentTarget.style.background = i === 0 ? 'rgba(251, 191, 36, 0.05)' : 'transparent'}>
                                                                            <td style={{ padding: '0.8rem 0.5rem', display: 'flex', alignItems: 'center', gap: '0.8rem', fontWeight: i === 0 ? 'bold' : 'normal', color: i === 0 ? 'var(--accent-gold)' : 'white' }}>
                                                                                <span style={{ color: 'var(--text-muted)', width: '20px', fontSize: '0.85rem', textAlign: 'center' }}>{t.rank}</span>
                                                                                <img src={t.flag} alt={t.team} style={{ width: '24px', height: '24px', objectFit: 'contain' }} onError={(e) => e.target.style.display = 'none'} />
                                                                                {t.team}
                                                                            </td>
                                                                            ...
                                                                            <td style={{ textAlign: 'center', color: 'var(--text-muted)' }}>{t.gd}</td>
                                                                        </tr>
                                                                    ))}
                                                                </tbody>
                                                            </table>
                                                    ...
                                                    {!ligaData.stats ? (
                                                        ...
                                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '3rem' }}>
                                                            ...
                                                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '2rem' }}>
                                                                ...
                                                                {/* Goleadores */}
                                                                {ligaData.stats.goleadores && ligaData.stats?.goleadores?.length > 0 && (
                                                                    <div className="glass-card" style={{ padding: '1.5rem', borderRadius: '12px' }}>
                                                                        ...
                                                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                                                                            {ligaData.stats.goleadores.map((s, i) => (
                                                                ...
                                                                {/* Asistencias */}
                                                                {ligaData.stats.asistencias && ligaData.stats?.asistencias?.length > 0 && (
                                                                    <div className="glass-card" style={{ padding: '1.5rem', borderRadius: '12px' }}>
                                                                        <h4 style={{ color: 'var(--accent-gold)', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem', borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '0.5rem' }}>
                                                                            🎯 Asistencias
                                                                        </h4>
                                                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                                                                            {ligaData.stats.asistencias.map((s, i) => (
                                                                                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.5rem', background: 'rgba(255,255,255,0.02)', borderRadius: '6px' }}>
                                                                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                                                                                        <span style={{ color: 'var(--text-muted)', fontSize: '0.9rem', width: '20px' }}>{i + 1}.</span>
                                                                                        <span style={{ display: 'flex', flexDirection: 'column' }}>
                                                                ...
                                                                {/* Amarillas */}
                                                                {ligaData.stats.amarillas && ligaData.stats?.amarillas?.length > 0 && (
                                                                    <div className="glass-card" style={{ padding: '1.5rem', borderRadius: '12px' }}>
                                                                        ...
                                                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                                                                            {ligaData.stats.amarillas.map((s, i) => (
                                                                                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.5rem', background: 'rgba(255,255,255,0.02)', borderRadius: '6px' }}>
                                                                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                                                                                        <span style={{ display: 'flex', flexDirection: 'column' }}>
                                                                                            <span>{s.name}</span>
                                                                ...
                                                                {/* Rojas */}
                                                                {ligaData.stats.rojas && ligaData.stats?.rojas?.length > 0 && (
                                                                    <div className="glass-card" style={{ padding: '1.5rem', borderRadius: '12px' }}>
