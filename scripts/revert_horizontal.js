...
// The replacement for the temporadas block: Horizontal Slider Navigation
const newTemporadasBlock = `
        <motion.div 
          ...
          >
            {/* ENCABEZADO Y NAVEGADOR HORIZONTAL (SCROLLERS) */}
            <div style={{ background: 'rgba(0,0,0,0.6)', padding: '1.5rem', borderBottom: '1px solid var(--glass-border)' }}>
               {/* SCROLLER 1: AÑOS */}
               <div className="hide-scrollbar" style={{ display: 'flex', gap: '1rem', overflowX: 'auto', marginBottom: '1.5rem', paddingBottom: '0.5rem' }}>
                 {allYears.reverse().filter(y => history.some(h => h.anio === y)).map(year => {
                    const isSelected = selectedYear === year || (selectedTournament && selectedTournament.anio === year);
                    
                    return (
                        <button
                          key={year}
                          onClick={() => {
                             // Si cambian el año, auto-seleccionar el primer torneo disponible de ese año
               ...
               {/* SCROLLER 2: TORNEOS DEL AÑO SELECCIONADO */}
               {selectedTournament && (
                   <div className="hide-scrollbar" style={{ display: 'flex', gap: '0.8rem', overflowX: 'auto' }}>
                     {history.filter(h => h.anio === (selectedTournament ? selectedTournament.anio : null)).map(t => (
                        <button
                          key={t.id}
                          onClick={() => setSelectedHistoryId(t.id)}
                          style={{
                             background: selectedHistoryId === t.id ? 'var(--accent-gold)' : 'transparent',
            ...
            <div className="hide-scrollbar" style={{ flex: 1, padding: '3rem', overflowY: 'auto' }}>
              {selectedTournament ? (
                ...
                <div style={{ padding: '4rem', textAlign: 'center', background: 'rgba(255,255,255,0.02)', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.05)' }}>
                  ...
                  <h3 style={{ color: 'white', fontSize: '1.5rem', marginBottom: '0.5rem' }}>Archivos Históricos de {selectedYear}</h3>
...
if (parts.length === 2) {
    ...
    if (endParts.length >= 2) {
        ...
        // Remove temporarily added state variables for calendar UX so there is no unused vars
        ...
        // We also need to restore selectedYear to state if it's missing (it was there originally in my earlier builds, but I might need to make sure)
        if (!parts[0].includes('const [selectedYear, setSelectedYear] = useState(')) {
             parts[0] = parts[0].replace(
        ...
        console.log("REVERTED TO HORIZONTAL NAVBAR.");
    } else {
